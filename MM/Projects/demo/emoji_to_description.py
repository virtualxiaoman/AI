    ...
    for file_path in sorted(folder.iterdir()):
        ...
        if file_path.is_file() and file_path.suffix.lower() in IMAGE_EXTENSIONS:
            ...
            try:
                ...
                # "输出的内容严格无多余文字。"
                # 4. 调用推理
                thinking, response = inference_engine.generate(img_path_str, prompt, max_new_tokens=max_new_tokens)

                # 5. 按要求输出内容
                print(f"图片路径: {img_path_str}")
                print(f"思考内容: {thinking}")  # todo 超字数了怎么办
...
if __name__ == "__main__":
    # 在这里输入你的图片文件夹路径
    my_folder = "../../../Datasets/Temp/Emoji"
    batch_process_emotions(my_folder)
